...
public class DragParachute extends AbstractPhysicalObject implements GraphicalObject {
    ...
    private Shape shape = Shape.createTriangle(100, 10);
	...
	p.addPoint(x, y);
	p.addPoint(x - deploymentStatus, y + deploymentStatus - weaknessStatus);
	p.addPoint(x, y - weaknessStatus * 1.2 + deploymentStatus * 1.2);
	p.addPoint(x + deploymentStatus, y + deploymentStatus - weaknessStatus);
	g.drawPolygon(p);
    }
